...
(function() {
	...
	var splitlayout = document.getElementById( 'splitlayout' ),
		...
		rightSide = splitlayout.querySelector( 'div.intro > div.side-right > div.intro-content > ul' ),
	...
	function init() {
		...
		rightSide.querySelector( '#about' ).addEventListener( eventtype, function( ev ) {
			reset();
			resetRightSection();
			classie.add( splitlayout, 'open-right');
			classie.add( splitlayout, 'open-right-about' );
		} );

		rightSide.querySelector( '#skill' ).addEventListener( eventtype, function( ev ) {
			reset();
			resetRightSection();
			classie.add( splitlayout, 'open-right');
			classie.add( splitlayout, 'open-right-skill' );



			setTimeout(function(){
				$('.chart').easyPieChart({
		        	barColor: 'rgba(53, 53, 53, 1)',
		    	   	size: '150',
		...
		rightSide.querySelector( '#experience' ).addEventListener( eventtype, function( ev ) {
			reset();
			resetRightSection();
			classie.add( splitlayout, 'open-right' );
			classie.add( splitlayout, 'open-right-experience' );
		} );

		rightSide.querySelector( '#contact' ).addEventListener( eventtype, function( ev ) {
			reset();
			resetRightSection();
			classie.add( splitlayout, 'open-right' );
			classie.add( splitlayout, 'open-right-contact' );
		} );
	...
	function resetRightSection(){
		classie.remove( splitlayout, 'open-right-about');
		classie.remove( splitlayout, 'open-right-skill' );
		classie.remove( splitlayout, 'open-right-experience' );
		classie.remove( splitlayout, 'open-right-contact' );
